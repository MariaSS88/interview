...
public class RoomsAndKeysGraph {
    /*There are N rooms and you start in room 0. Each room has a distinct number in 0, 1, 2, ..., N-1, and each room may have some keys to access the next room.
        ...
        Note: 1 <= rooms.length <= 1000 0 <= rooms[i].length <= 1000 The number of keys in all rooms combined is at most 3000.*/
    ...
    public void visit (List<List<Integer>> rooms, int roomIndex, Set<Integer> visited){
        if(visited.contains(roomIndex)) return;
    ...
    public static void main(String[] args){
        ...
        RoomsAndKeysGraph test = new RoomsAndKeysGraph();
